...
function buscarMedicos(busqueda, regex) {
  ...
  return new Promise((resolve, reject) => {
    Medico.find()
      .or([{ 'nombres': regex }, { 'apePaterno': regex }, { 'apeMaterno': regex }])
      .populate('usuario', 'nombres apePaterno apeMaterno email role')
      .populate('hospital')
      .exec((err, medicos) => {
        if (err) {
...
function buscarUsuarios(busqueda, regex) {
  ...
  return new Promise((resolve, reject) => {
    ...
    Usuario.find({}, 'nombres apePaterno apeMaterno email role')
      .or([{ 'nombres': regex }, { 'apePaterno': regex }, { 'apeMaterno': regex }, { 'email': regex }])
      .exec((err, usuarios) => {
        if (err) {
